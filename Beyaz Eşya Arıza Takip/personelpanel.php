...
if(!isset($_SESSION["pid"])) {
    ...
    $sql = "SELECT ar.*, u.ad, u.soyad FROM arizarandevu ar
            ...
            WHERE ar.durum = 0 AND ar.personel = '$personelPid' ORDER BY ar.tarih ASC";
...
<div class="wrapper">
    ...
    <div id="content">
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
             ...
             <b><h1 align="center">  GELEN ARIZALAR <input type="text" class="form-control" id="searchInput" placeholder="Arıza Ara"> </h1> </b>
                </div>
            </nav> 
...
if (isset($result) && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "</p>";
        // Formlar
        echo "<form action='personelariziptal.php' method='post'> 
                    <input type='hidden' name='arizaİd' value='" . $row["arizaİd"] . "'>
                    <button type='submit' class='btn btn-danger'>Arızayı İptal Et</button>
                </form>";
        echo "<br>";
        echo "<form action='arizapersonelislem.php?arizaİd=" . $row["arizaİd"] . "' method='post'> 
                    <button type='submit' class='btn btn-primary'>Arızanın Bilgilerini Tamamlama</button>
                </form>";
        echo "<br>";
        echo "<form action='personelarizatamam.php?arizaİd=" . $row["arizaİd"] . "' method='post'> 
                    <button type='submit' class='btn btn-success'>Arızayı Tamamlandı Olarak İşaretle</button>
                </form>";
        echo "</div>";
